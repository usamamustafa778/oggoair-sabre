// Retrieve a specific Revolut order by ID
...
export default async function handler(req, res) {
    ...
    try {
        ...
        // Retrieve order from Revolut API
        ...
        if (!response.ok) {
            return res.status(response.status).json({
                message: data?.message || "Failed to retrieve Revolut order",
                error: data,
            });
        }
        ...
        console.error("Revolut retrieve order error:", error);
        return res.status(500).json({
