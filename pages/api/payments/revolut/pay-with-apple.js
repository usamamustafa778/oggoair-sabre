// Create a Revolut payment using an Apple Pay token
...
export default async function handler(req, res) {
    ...
    try {
        ...
        // Revolut: Create a payment with Apple Pay
        ...
        if (!response.ok) {
            return res.status(response.status).json({
                message: data?.message || "Failed to create Revolut Apple Pay payment",
                error: data,
            });
        }
        ...
        console.error("Revolut Apple Pay error:", error);
